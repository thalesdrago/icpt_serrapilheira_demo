...
round(3.14159)
args(round)
?round  #one question mark searchs an specific function, two search for a word inside the documentation of functions
...
weight_g[c(TRUE, FALSE, FALSE, TRUE)] # instead using this, we creat a logic operation and than use it to select the data
weight_g > 35 # logic operation
weight_g[weight_g > 35] #logic operation being used to select data
...
## subset with grep ------------------------------------------------------------
grepl("^d", more_animals) #searching animals that start with d, resulting in a logical output
grep("^d", more_animals) #searching animals that start with d, resulting in a numeric output
more_animals[grepl("^d", more_animals)]

## ----recycling2---------------------------------------------------------------
animals
more_animals
animals == more_animals # two == asks if the objetcs are the same
setdiff(animals, more_animals) # check what differ between objects

## ----na-----------------------------------------------------------------------
heights <- c(2, 4, 4, NA, 6)
mean(heights) #it returns NA
max(heights)
mean(heights, na.rm = TRUE) # argument that remove the NA, allowing to run the function disregarding NA
max(heights, na.rm = TRUE)  # several functions have this same argument used to remove the NA
...
levels(animals_cls)
levels(animals_cls) <- c("small", "medium", "large") # change the order to the factors at your pleasure
animals_cls
...
surveys <- read.csv(file = "./data/raw/portal_data_joined.csv") #using a relative path
